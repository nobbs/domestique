...
│   ├── photon/                     Photon geocoding HTTP adapter
...
| activity | decoded activity FIT values and their validation, polling a target's activity summaries into the store, and composing the prompt one ride's analysis is asked with and the schema its answer is held to | SQL, Wahoo URLs, OAuth, scheduling, HTTP routing, how the `claude` executable is run |
...
| photon | the geocoding HTTP adapter: naming one coordinate, finding places by name, decoding either reply, and its own bounded in-memory cache of past names | which coordinates are worth asking about, or what the UI draws |
...
| claude | running the bundled `claude` executable for one prompt over `os/exec`: the child's environment and arguments, a bounded timeout, decoding its JSON answer and the document it validated against a caller's schema, mapping its failures to a stable category | what a prompt says, SQL, HTTP routing, scheduling, anything about rides |
