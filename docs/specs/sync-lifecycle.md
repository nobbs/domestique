...
The provider is the upstream a route's source route ID was issued by.
VeloPlanner is the only provider a route has ever come from; naming it
explicitly is what lets a second provider issue its own source route IDs later
without colliding with VeloPlanner's.
...
A VeloPlanner route's external ID renders exactly as it always has:
...
can never affect the trusted inventory, target route mappings, OAuth state, or a
...
request. A route whose content hash is unchanged is **not rewritten**, which
keeps an unchanged library from rewriting the whole cache every hour; rows whose
route has left the inventory are pruned.

It is deliberately a separate record from the route itself. The trusted route
set backs the deletion guard and is replaced wholesale each run, whereas this cache
...
the geometry, it needs data no run holds: the ways lying under each route, read
...
The whole inventory is walked in one pass. A local index answers a route in a few
milliseconds, so there is nothing to spread over several runs; a route already
...
A pass with no index behind it does nothing at all — no route is recorded as
...
A route that fails does not end the pass. Each route gets its own attempt
whatever happened to the one before, so one unreadable route cannot starve every
route behind it — the inventory is always walked in the same order, and an early
...
log line carrying the counts and whether it ran to the end — never a route name,
never geometry, never anything the endpoint said. `GET /v1/status` reports two
counts alongside it: how many stored routes carry a classification of the
...
simply waiting its turn. Without the second, a route that fails every pass would
be indistinguishable from one nobody has asked about — both absent from the
first alike. A route whose
...
target. It reclassifies the routes already stored against the local surface
...
three for that route and starts a synchronization of both halves, so the route is
...
start. A run already in flight may be past that route or may not include it, so
the request waits for a pass that will honour it. It is consumed by the pass that
honours it, so it is met exactly once.

A request for a route that is not in the stored inventory is refused. There would
...
Each route is recorded as its own write succeeds, so the next scheduled run
...
library whose missing routes are meant to be deleted, and the difference is not
...
from being read, and never widens into a deletion of another source's routes.
...
by the gate below — keeps the routes it was last known to have, and those
routes remain part of the merged inventory the target half reconciles from,
...
own prior route count: a source that previously had routes and now reports
...
A route's identity already carries the provider that issued it, so two sources
reporting the same source route ID and stage order store as two distinct routes, not
...
blocked, each against its own provider, plus the count of routes each
...
run has always used, and no per-route detail crosses this boundary: a source
...
establishes the same fact a per-route lookup did — what the target actually
holds right now, by external ID — for every route at once, so a library where
...
per-route lookup spent it in proportion to library size.
...
`routes` is the aggregate of the per-target counts above. That is the whole of the progress reported: it is
derived from local state alone, so watching a run costs no provider call, and
it is counts only, so no route is named.

`surface` counts how many stored routes carry a classification measured against
the geometry they hold now; a classification of an earlier shape of a route does
not count, because it describes a line the map no longer draws.

`incomplete` counts routes the most recently completed classification pass could
not classify, and is what tells those apart from routes simply waiting their
turn — a route that fails every pass would otherwise look exactly like one
...
`converged` and the per-target `convergence`, `routes`, and `last_run` answer
...
routes the source phase last replaced wholesale — against that bound, derived
...
revision, last synchronised time, and per-target state. It does not include a FIT payload,
...
### Superseded route addresses

Two earlier shapes of a route's address still resolve, each redirecting with 308
straight to the current one, preserving method and body. 308 rather than 301 or
302 is what keeps the reprocess request's `POST` and body intact across the
redirect.

`GET /v1/providers/{provider}/routes/{source-route-id}/routes/{stage-order}`,
its `/geometry`, and its `/reprocess` are the shape from before *route* named
the unit. They redirect to the same route under the same provider.

`GET /v1/routes/{source-route-id}/routes/{stage-order}`, its `/geometry`, and
its `/reprocess` are the shape from before a second provider existed. They
redirect to the same route under `veloplanner`, skipping the intermediate shape
so an old address still costs one hop. The browser address
`GET /routes/{source-route-id}/{stage-order}` redirects the same way, to
