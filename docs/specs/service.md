...
- The plan endpoints (all admin-only) are the one place a route is written.
  ...
  when the engine asked to be retried later or was too busy to answer, and
  `routing_failed` otherwise, and nothing of the engine's response. A plan
  carries no credential, no rider's data, and
...
the kind of ride, a headline, a summary of one short paragraph, its effect on
