...
`golangci-lint`, `prek`, `yq`, Actionlint, Gitleaks, Markdownlint, ShellCheck,
...
| `internal/*/contract/openapi.gen.go` | Generated Go models, for the served listener and the readiness one. Generator freshness and contract tests verify the source of truth; branch coverage would only grade generator output. |
