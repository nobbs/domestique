...
// is nil (no speed series to derive them from) held time is judged against
// the whole recording instead, which is the ride's only account of when it
// moved. A non-nil, empty movingIntervals -- a series that named no moving
// time at all -- is read as no coverage rather than that fallback. Capped at
// 1. False for a series with nothing to hold at all.
...
func heldSeconds(samples []Sample, movingIntervals []measure.Interval) float64 {
	// Nil is "no speed series to judge by" -- measure.MovingIntervals's own
	// contract -- and falls back to the whole recording; a series that named
	// no moving time at all comes back non-nil and empty, and must read as
	// zero held seconds rather than the same fallback.
