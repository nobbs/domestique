...
func newHandlerWithSync(t *testing.T, oauthService OAuth, state State, syncRuns Sync) *Handler {
	...
	handler, err := New(
		&Options{
			...
			Snapper:        &fakeSnapper{},
			PlanDeliveries: &fakePlanDeliveries{},
		},
...
// ActivityRideLoads reports the ride loads the test gave this target.
func (s *fakeState) ActivityRideLoads(
	_ context.Context, targetID string,
) ([]trainingload.RideLoad, error) {
	if s.rideLoadsErr != nil {
		return nil, s.rideLoadsErr
	}

	return s.rideLoads[targetID], nil
}
...
// for the window from one read for the window before it.
func (s *fakeState) PowerCurve(
	...
	s.powerCurveWindows = append(s.powerCurveWindows, [2]time.Time{from, to})
...
// ActivitySeries reports the samples the test gave this target's activity,
// keyed the way the store scopes it.
func (s *fakeState) ActivitySeries(
	_ context.Context, targetID string, id int64,
) ([]activities.SampleRow, error) {
	if s.sampleRowsErr != nil {
		return nil, s.sampleRowsErr
	}
