...
describe("SourceRouteLink", () => {
  it("opens the source route, named so its purpose is clear without seeing it", () => {
    render(<SourceRouteLink provider={VELOPLANNER} baseUrl={BASE} routeId={4212} />);
  ...
  it("renders nothing at all when the service names no provider", () => {
    const { container } = render(
      <SourceRouteLink provider={VELOPLANNER} baseUrl={undefined} routeId={4212} />,
    );
  ...
  it("renders nothing rather than a dead link for a base URL it cannot use", () => {
    render(<SourceRouteLink provider={VELOPLANNER} baseUrl="not-a-url" routeId={4212} />);

    expect(screen.queryByRole("link")).toBeNull();
  });

  // A configured base URL is not enough on its own: a provider whose path
  // convention is not known must not get a guessed link either.
  it("renders nothing for a provider with no known path convention", () => {
    render(<SourceRouteLink provider="komoot" baseUrl={BASE} routeId={4212} />);
