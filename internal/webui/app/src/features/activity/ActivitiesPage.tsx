/**
 * Activities: what the rider has actually ridden, their whole recorded history.
 *
 * Every other page is about routes the service holds for them; this is the one
 * about rides they have already done. One range and one ground filter serve two
 * views: the overview, weeks and months totalled, and the rides themselves.
 */
import {
  IconBike,
  IconCalendarMonth,
...
import { RideCalendar, riddenDays } from "../../components/RideCalendar";
...
/** Each ground chosen paints the selector in its own colour; both split on the diagonal. */
const GROUNDS: ReadonlyArray<{ key: Ground; label: string; fill: string }> = [
  {
    key: "both",
    label: "Both",
    fill: "linear-gradient(135deg, var(--ground-outdoor) 0 50%, var(--ground-indoor) 50% 100%)",
  },
  { key: "outdoor", label: "Outdoor", fill: "var(--ground-outdoor)" },
  { key: "indoor", label: "Indoor", fill: "var(--ground-indoor)" },
];
...
export function ActivitiesPage() {
  const view = useMatch("/activities/rides") ? "rides" : "overview";
  ...
  // The day's first ride in the list, brought into view and focused, and every
  // ride that day tinted for a moment so the eye finds them.
  const showDay = (day: string) => {
    const ride = document.querySelector<HTMLElement>(`[data-day="${day}"]`);
      ...
      <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-5">
        ...
        {recorded.length > 0 ? (
          <Segmented
            ...
            items={VIEWS}
            value={view}
            onChange={(next) => navigate(next === "rides" ? "/activities/rides" : "/activities")}
        ...
        {isPending ? (
          ...
          <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]">
            ...
            <RideCalendar
              rides={recorded}
              zone={zone}
              listed={new Set(riddenDays(activities, zone).keys())}
              onDay={showDay}
            />
          </div>
        ) : (
