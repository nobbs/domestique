...
describe("mergeCoverage", () => {
  const file = `${APP}/src/features/routes/RoutePanel.tsx`;
...
describe("sourcePath", () => {
  ...
  it("ignores the dev server's cache buster", () => {
    const map = { version: 3, sources: ["RoutePanel.tsx"], names: [], mappings: "" };

    expect(
      sourcePath("http://localhost:5173/src/features/routes/RoutePanel.tsx?t=1730000", map, APP),
    ).toBe(`${APP}/src/features/routes/RoutePanel.tsx`);
  });
