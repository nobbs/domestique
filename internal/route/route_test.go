...
func TestOwnsExternalID(t *testing.T) {
	...
	stage, err := NewRoute(ProviderKomoot, 7, 1, "2026-08-23", "Ride", "", []Point{
...
func TestNewStageCreatesImmutableStage(t *testing.T) {
	...
	stage, err := NewRoute(ProviderVeloPlanner, 17, 2, "2026-08-16", "Morning ride", "Climb", geometry, "hash")
	require.NoError(t, err)
...
func TestStageDistanceMetres(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			stage, err := NewRoute(ProviderVeloPlanner, 1, 1, "revision", "route", "", test.geometry, "hash")
			require.NoError(t, err)
...
func TestStageBounds(t *testing.T) {
	stage, err := NewRoute(ProviderVeloPlanner, 1, 1, "revision", "route", "", []Point{
...
func elevationTestStage(t *testing.T, elevations []float64, latitudeStep float64) Route {
	t.Helper()
	...
	stage, err := NewRoute(ProviderVeloPlanner, 1, 1, "revision", "route", "", geometry, "hash")
	require.NoError(t, err)
...
func TestStageElevationStatisticsNeedACompleteProfile(t *testing.T) {
	summit := 200.0
	partial, err := NewRoute(ProviderVeloPlanner, 1, 1, "revision", "route", "", []Point{
...
func TestNewStageRejectsInvalidIdentityAndGeometry(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "provider", provider: "", sourceRouteID: 1, order: 1, geometry: validGeometry, want: "provider"},
		{name: "route ID", provider: ProviderVeloPlanner, sourceRouteID: 0, order: 1, geometry: validGeometry, want: "route ID"},
		{name: "stage order", provider: ProviderVeloPlanner, sourceRouteID: 1, order: 0, geometry: validGeometry, want: "stage order"},
		{name: "short geometry", provider: ProviderVeloPlanner, sourceRouteID: 1, order: 1, geometry: validGeometry[:1], want: "at least two"},
		{name: "longitude", provider: ProviderVeloPlanner, sourceRouteID: 1, order: 1, geometry: []Point{{Longitude: 181, Latitude: 49}, {Longitude: 8, Latitude: 49}}, want: "longitude"},
		{name: "latitude", provider: ProviderVeloPlanner, sourceRouteID: 1, order: 1, geometry: []Point{{Longitude: 8, Latitude: math.NaN()}, {Longitude: 8, Latitude: 49}}, want: "latitude"},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := NewRoute(test.provider, test.sourceRouteID, test.order, "revision", "route", "stage", test.geometry, "hash")
