...
func TestMigration063DownKeepsEveryPlan(t *testing.T) {
	t.Parallel()
	dbPath := filepath.Join(t.TempDir(), "plan-cues-rollback.db")
	migration, closeFn, err := openMigrator(dbPath, migrationFiles, "migrations")
	require.NoError(t, err)
	defer closeFn()
	...
	_, err = database.ExecContext(t.Context(), `
		...
		VALUES (123456789, 'Sunday loop', 'gravel', '[[8.4,49.0],[8.5,49.1]]', x'5b5d',
			1000, 50, 1, 3, 1700000000000000000, 1700000000000000001, '[[100,200]]',
			'[{"turn":"left","metres":10}]', 1)`)
	require.NoError(t, err)

	require.NoError(t, migration.Migrate(62))

	var name, pushing string
	require.NoError(t, database.QueryRowContext(t.Context(),
		`SELECT name, pushing FROM plans WHERE id = 123456789`).Scan(&name, &pushing))
	assert.Equal(t, "Sunday loop", name)
	assert.Equal(t, "[[100,200]]", pushing, "the walking windows survive the rollback")
	var cueColumns int
	require.NoError(t, database.QueryRowContext(t.Context(),
		`SELECT COUNT(*) FROM pragma_table_info('plans') WHERE name IN ('turns', 'cues')`).Scan(&cueColumns))
	assert.Zero(t, cueColumns, "the cue columns must be gone after rollback")
