...
#   * the bundled claude executable starts, with no network, as that user on a
...
#
# A failure prints the container log with the encryption key of this run
# replaced, so a failure that happens before that assertion cannot print it
# either.
#
...
# The browser origin, which has to be an absolute HTTPS URL. Unroutable, and
# only ever named, never fetched: no request here carries an Origin header.
...
# The one secret the host still hands the process, written by this script. It
# has to be 32 bytes of base64url to load at all, and it encrypts a database
# this run creates and deletes.
...
# The container log is dumped on failure and only on failure, with the key this
# run mounted replaced on the way out. The service is what should keep a secret
# value out of a log line, and that is asserted below over the log as it really
# is — but a run can fail long before it reaches that assertion, including while
# starting, so what a failure prints is filtered rather than trusted.
redacted() {
  sed -e "s|${ENCRYPTION_KEY}|[redacted]|g"
}

cleanup() {
  local status=$?
  if [[ "${status}" -ne 0 ]] && "${DOCKER}" inspect "${CONTAINER}" >/dev/null 2>&1; then
    printf 'container-smoke: the container log follows, with the encryption key redacted\n' >&2
...
cat > "${CONFIG_FILE}" <<EOF
# Generated by dev/container-smoke.sh. The file carries only what the host has
# to hand the process: where to listen, which tenant to verify a session
# against, and where the state lives.
...
# Ready on a database this run created. Nothing is configured yet, and that is
# the deployed shape of a first start: a service with no target slots is running
# exactly as deployed, waiting for a browser, so the probe reports it ready
# rather than rolling the deployment back before anyone can configure it.
...
# that the service keeps this out, not that this script can hide it.
container_log="$("${DOCKER}" logs "${CONTAINER}" 2>&1)"
[[ "${container_log}" != *"${ENCRYPTION_KEY}"* ]] ||
