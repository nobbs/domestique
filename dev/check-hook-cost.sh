...
# it stays fast.
#
# A hook that scans the whole tree on every commit, or that runs work
# `mise run check` already owns, becomes slow enough that developers bypass it —
# and a bypassed hook protects nothing. So two structural rules hold in
# prek.toml:
#
...
#     to `mise run check` and to GitHub Actions.
#
...
#
# `mise run` is matched, and `mise exec` deliberately is not: a hook reaching a
...
awk -v expensive="${EXPENSIVE}" -v wide="${WIDE}" -v config="${CONFIG}" '
	...
	function flush(  invocation) {
		...
		if (invocation ~ expensive) {
			report(sprintf("hook %s runs `%s`, which belongs to `mise run check`, not to a commit hook", id, invocation))
		}
		if (invocation ~ wide) {
